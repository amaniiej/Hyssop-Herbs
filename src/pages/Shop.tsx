...
export default function Shop() {
  ...
  // ── All products from JSON (typed, category defaulted if missing) ──
  const allProducts: Product[] = (productsData as Product[]).map(p => ({
    ...p,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // 1. Save order to Supabase (this is the source of truth — never lost)
      ...
      // 2. Show success immediately — don't make customer wait for email
      setSubmitted(true);
      if (cartSource === "cart") setCart([]);

      // 3. Fire email notification in background (fire-and-forget)
      // If this fails, the order is still saved in Supabase — check dashboard
      // "to" is NOT sent — it is hardcoded in the Edge Function (prevents open mail relay)
      ...
      }).catch(err => console.warn("Email notification failed (order is still saved):", err));
